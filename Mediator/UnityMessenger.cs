...
namespace Core.DefinitionLibrary
{
    /// <summary>
    ...
    /// </summary>
    public class UnityMessenger : IMediator
    {
        ...
        public void UnRegister(string message, Action callback)
        {
        ...
        public void UnRegister<T>(string message, Action<T> callback)
        {
        ...
        void UnRegister(string message, Delegate callback, Type parameterType)
        {
            if (String.IsNullOrEmpty(message))
                throw new ArgumentException("'message' cannot be null or empty.");
        ...
        public void NotifyColleagues(string message, object parameter)
        {
            ...
            if (actions != null && parameter != null)
            {
        ...
        private class MessageToActionsMap
        {
            ...
            internal void RemoveAction(string message, object target, MethodInfo method, Type actionType)
            {
                if (message == null)
                {
                    throw new ArgumentNullException("message");
                ...
                lock (_map)
                {
                    if (!_map.ContainsKey(message))
                    {
                        //throw new ArgumentException("message not contained");
                    }

                    // only remove registers from the same source
                    List<WeakAction> WAToRemove = new List<WeakAction>();
                    foreach (WeakAction WA in _map[message])
                    {
                        if (WA._targetRef.Target == target)
                        {
                            WAToRemove.Add(WA);
                        }
                    }

                    foreach (WeakAction WA in WAToRemove)
                    {
